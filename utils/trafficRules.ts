 ...
 * Horários: 7h-10h e 17h-20h (Validamos o dia completo para segurança no agendamento)
 */
export const checkSPRodizio = (plate: string, date: Date = new Date()) => {
  const dayOfWeek = date.getDay(); // 0 (Dom) a 6 (Sab)
  
  // Limpa a placa e pega o ÚLTIMO DÍGITO NUMÉRICO (independente de ser Mercosul ou Antiga)
  const numbersOnly = plate.replace(/\D/g, '');
  const lastDigit = parseInt(numbersOnly.slice(-1));
